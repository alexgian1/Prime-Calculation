...
void determ(long long unsigned start, long long unsigned stop){
	...
	for (current=start+1; current<=stop; current+=2){  /*Skips even numbers*/
		curDivs=0;
		int d;
		for (d=3;d*d<=current;d+=2){  /*divider testing ends at the sq.root of current/even numbers skipped */
			if (current%d==0){
				curDivs++;
				break;
			}	
...
void milRab(long long unsigned start, long long unsigned stop){
	double Tstart,Tstop;  /*Counting time elapsed*/
	int primeCount;
	Tstart = ((float) clock())/CLOCKS_PER_SEC;
	primeCount = 0; 
	...
	unsigned int n;
	for (n=start+1;n<=stop;n+=2){  /*The described algorithm begins here.
